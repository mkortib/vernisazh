...
<?php
	...
	public function display_rows() {
		...
		foreach ( (array) $this->items as $plugin ) {
			...
			if ( !empty( $plugin['ics']['svg'] ) ) {
				$plugin_icon_url = $plugin['ics']['svg'];
			} elseif ( !empty( $plugin['ics']['2x'] ) ) {
				$plugin_icon_url = $plugin['ics']['2x'];
			} elseif ( !empty( $plugin['ics']['1x'] ) ) {
				$plugin_icon_url = $plugin['ics']['1x'];
			} else {
				$plugin_icon_url = $plugin['ics']['default'];
			}
